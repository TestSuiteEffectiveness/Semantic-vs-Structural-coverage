...
public class FastMathTest {
    ...
    private static int[] testInputs ={3,4,8,9,12,16,19,21,26,29,30,35,36,45,46,55,56,65,68,75,80,86,90,91,94,108,109,110,112,113,114,115,118,119,120,123,125,128,130,131,132,133,134,139,140,145,146,148,149,150,156,161,167,180,183,187,190,197}; 	
...
public static double  exmp1v=-0.1;
...
@BeforeClass
   public static void setUp() {
	...
	try{
		results_writer = new FileWriter("./outputTest/T13.txt");
		} catch (IOException e) {
		}
		test_count=1;
	...
	public static void readFile(){
		...
		try {
      File myObj = new File("./outputTest/T0.txt");
...
double tst=1;
        for (int i = 0; i < 1000; i++) {
            ...
            /* double x = 3.0 / 512.0 * i - 3.0; */
			/*try{
				printTestOutput(test_count,model.atan(x));// test 
				 tst = FastMath.atan(x);

			}catch(final Exception e) {

        printTestOutput(test_count,Exception.class);// test 
		tst=1;
  ...
  //@Test
    public void testExpm1Accuracy() throws Exception {
            ...
            for (int j = 0; j < testInputs.length; j++) {
				//double x = 1.0 + j/1024.0/2.0;
				
				System.out.println(testInputs[j]+model.expm1(exmp1v,null));
if (  testInputs[j]>=9) {
				
				try{

			   /*printTestOutput(testInputs[j],"class java.lang.Exception");

else 	*/	   
			   printTestOutput(testInputs[j], model.expm1(exmp1v,null));
				
            
           }catch(final Exception   e) {
			   System.out.println();
					//printTestOutput(testInputs[j], e.getMessage());
...
//////////////////////////////////
    ...
    //@Test
    public void testExpm1Failures() {
		 // Case 1: x too large (overflow scenario)
        //assertThrows(MathException.class, () -> expm1(800.0));
		// find failed cases in each test 
		for (int i = 0; i < 9; i++) {
			
			if (testInputs[i]==0)
			{
				try{
				                printTestOutput(0,model.expm1(800.0,null));// test 
				 }catch(final Exception e) {
					printTestOutput(0, Exception.class);
				}
				}// if
 	exmp1v=exmp1v+0.001;
		...
		/////////////////////////
		 // Case 2: x too small (underflow scenario)
        //assertThrows(MathException.class, () -> expm1(-800.0));	
 
	
if (testInputs[i]==1)
			{				try{
				printTestOutput(1,model.expm1(-800.0,null));// test 
				 }catch(final Exception e) {
					printTestOutput(1, Exception.class);
				}
				}
				exmp1v=exmp1v+0.001;
...
///////////////////////////////			
  // Case 3: Very large positive x, beyond the boundary
       // assertThrows(MathException.class, () -> expm1(710.0));			
				
				if (testInputs[i]==2)
			{
				
				try{
					printTestOutput(2,model.expm1(710.0,null));// test
					}catch(final Exception e) {
					printTestOutput(2, Exception.class);
	...
	/////////////////////////////////////			
        ...
        // Case 4: Very large negative x, beyond the boundary
      //  assertThrows(MathException.class, () -> expm1(-710.0));
				
				if (testInputs[i]==3)
			{
				try{
				printTestOutput(3,model.expm1(-710.0,null));// test 
					}catch(final Exception e) {
					printTestOutput(3, Exception.class);
	...
	//////////////////////////////////////////////////////////////			
        ...
        // Case 5: NaN as input
       // assertThrows(MathException.class, () -> expm1(Double.NaN));

if (testInputs[i]==4)
			{				try{
				printTestOutput(4,model.expm1(Double.NaN,null));// test 
					}catch(final Exception e) {
					printTestOutput(4, Exception.class);
...
//////////////////////////////////	   
	...
	// Case 6: Infinite positive input
       // assertThrows(MathException.class, () -> expm1(Double.POSITIVE_INFINITY));

if (testInputs[i]==5)
			{					try{
				printTestOutput(5,model.expm1(Double.POSITIVE_INFINITY,null));// test 
					}catch(final Exception e) {
					printTestOutput(5, Exception.class);
	...
	////////////////////////////////////////
        ...
        // Case 7: Infinite negative input
       // assertThrows(MathException.class, () -> expm1(Double.NEGATIVE_INFINITY));

if (testInputs[i]==6)
			{				try{
				printTestOutput(6,model.expm1(Double.NEGATIVE_INFINITY,null));// test 
					}catch(final Exception e) {
					printTestOutput(6, Exception.class);
...
////////////////////////////////////	
	 // Case 8: Input causing extreme precision loss
      //  assertThrows(MathException.class, () -> expm1(1.7976931348623157E308)); // Double.MAX_VALUE

	
if (testInputs[i]==7){
				
				try{
				printTestOutput(7,model.expm1(1.7976931348623157E308,null));// test 
					}catch(final Exception e) {
					printTestOutput(7, Exception.class);
	...
	exmp1v=exmp1v+0.001;
        ...
        // Case 9: Very small positive x, underflow scenario
        //assertThrows(MathException.class, () -> expm1(-1.7976931348623157E308)); // -Double.MAX_VALUE

if (testInputs[i]==8){
				try{
				printTestOutput(8,model.expm1(-1.7976931348623157E308,null));// test 
					}catch(final Exception e) {
					printTestOutput(8, Exception.class);
	...
	exmp1v=exmp1v+0.001;
        ...
        // Case 10: Extremely small x, close to zero but negative
      //  assertThrows(MathException.class, () -> expm1(-1.0e-324)); // Smallest subnormal number
		/*try{
                printTestOutput(test_count, model.expm1(exmp1v, null));
            }
            exmp1v = exmp1v + 0.001;
        }
				printTestOutput(test_count,model.expm1(-1.0e-324,null));// test 

                printTestOutput(test_count, Exception.class);
printTestOutput(test_count, e.getClass());// test 
...
//printTestOutput(test_count,model.acos(x));// test 
  ...
  //  @Test
    ...
    private Dfp tanh(Dfp x) {
	 ...
	 // @Test
    public void testSinhFailure() {
      
               //case 0
			   try{
				   printTestOutput(test_count,model.sinh(Double.NaN));// test 
				 }catch(final Exception e) {
				   printTestOutput(0, Exception.class);
				}
				...
				//case 1
			   try{
				   printTestOutput(test_count,model.sinh(-0.0));// test 
				 }catch(final Exception e) {
					printTestOutput(1, Exception.class);
				}
				...
				//case 2
			   try{
				   printTestOutput(test_count,model.sinh(-20));// test 
				 }catch(final Exception e) {
					printTestOutput(2, Exception.class);
				}
				...
				//case 3
			   try{
				   printTestOutput(test_count,model.sinh(Double.POSITIVE_INFINITY));// test 
				 }catch(final Exception e) {
					printTestOutput(3, Exception.class);
				}
				...
				//case 4
			   try{
				   printTestOutput(test_count,model.sinh(Double.NEGATIVE_INFINITY));// test 
				 }catch(final Exception e) {
					printTestOutput(4, Exception.class);
				}
  ...
  for (int i=5; i<200; i++) {
		try{
			printTestOutput(test_count, model.sinh(i));
		 }catch(final Exception   e) {
			   System.out.println();
		 }
		}
  ...
  //  @Test
   ...
   // @Test
	...
	//amani 10-6
	 ...
	 @Test
    public void test01_TanhFailureCases()throws Exception {
        ...
        double[] edgeCases = {
		 ...
		 // select which test_count indices to consider (example: 1, 3, 5)
    Set<Integer> selectedTests = new HashSet<Integer>(Arrays.asList(
        new Integer[] {1,2,9,11}   // change numbers as you like
    ));
for (double x : edgeCases) {
	  if (!selectedTests.contains(test_count)) {
		  test_count++;
            continue; // skip all tests not in the list
        }
           try {
...
public void test02_TanhAccuracyAA() throws Exception {
    ...
    Set<Integer> selectedTests = new HashSet<Integer>(Arrays.asList(
    new Integer[] {
        14,15,16,17,18,20,22,24,26,28,30,34,35,36,37,38,39,41,43,46,47,48,49,50,51,52,54,58,59,60,62,64,66,70,71,72,75,79,80,81,83,84,85,86,87,88,90,93,94,96,97,98,99,101,104,106,107,108,109,111,113,114,115,116,118,120,121,122,124,125,129,132
    }
));
...
public void test02_TanhAccuracyAA() throws Exception {
    ...
    for (int test_count = 13; test_count < 133; test_count++) {
        double x = -3.0 + 6.0 * rand.nextDouble(); // random between -3 and +3

        try {
            double ref = FastMath.tanhA(x); // using your updated function with exceptions
            printTestOutput(test_count, ref);
        } catch (Exception e) {
            printTestOutput(test_count, Exception.class);
        }
